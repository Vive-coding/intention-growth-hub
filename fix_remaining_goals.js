...
/**
 * Fix remaining problematic goals
 * Run with: node fix_remaining_goals.js
 */
...
async function fixRemainingGoals() {
  try {
    console.log('🔧 Fixing remaining problematic goals...\n');

    const mainUser = '2703277e-97db-4096-bd30-6798dd0470d9';
    
    // Find all goals with problematic values
    console.log('1. Finding all goals with problematic values...');
    ...
    problematicGoals.forEach((goal, i) => {
      console.log(`   ${i + 1}. [${goal.metric_name}] ${goal.title} - ${goal.current_value}/${goal.target_value} = ${goal.calculated_progress}%`);
    });
    ...
    console.log('\n2. Fixing remaining problematic goals...');
    ...
    console.log('\n✅ All goals fixed and snapshots recalculated!');

  } catch (error) {
    console.error('❌ Error fixing remaining goals:', error);
  } finally {
    await sql.end();
  }
}
